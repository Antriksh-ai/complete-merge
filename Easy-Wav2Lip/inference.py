...
def datagen(frames, mels):
    ...
    # Detect faces based on the provided bounding box or using face detection
    if args.box[0] == -1:
        if not args.static:
            face_det_results = face_detect(frames)  # Detect face(s) for each frame
        else:
            face_det_results = face_detect([frames[0]])  # Detect face only once for static
    else:
        print("Using the specified bounding box instead of face detection...")
        y1, y2, x1, x2 = args.box
        face_det_results = [[f[y1:y2, x1:x2], (y1, y2, x1, x2)] for f in frames]
    ...
    for i, m in enumerate(mels):
        ...
        # Check if a face was detected in the current frame
        if idx < len(face_det_results) and face_det_results[idx] is not None:
            face, coords = face_det_results[idx].copy()
            face = cv2.resize(face, (args.img_size, args.img_size))
            ...
            print(f"Skipping frame {idx} due to no detected face.")
            continue

        # Yield batches once they reach the specified size
        if len(img_batch) >= args.wav2lip_batch_size:
            img_batch, mel_batch = np.asarray(img_batch), np.asarray(mel_batch)
    ...
    # Yield remaining frames after the loop
    if len(img_batch) > 0:
        img_batch, mel_batch = np.asarray(img_batch), np.asarray(mel_batch)
