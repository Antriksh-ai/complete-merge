...
def lip_sync(video_file, vocal_file):
    ...
    output_filename = os.path.splitext(os.path.basename(video_file))[0] + "_lipsynced.mp4"

    cmd = [
        sys.executable, "inference.py", "--face", video_file, "--audio", vocal_file,
        "--outfile", temp_output, "--checkpoint_path", "checkpoints/Wav2Lip.pth"
    ]
...
with gr.Blocks() as app:
    with gr.Row():
        with gr.Column():
            ...
            video_input = gr.File(label="Step 4: Upload Video for Lip-Sync", visible=False)
            lip_sync_btn = gr.Button("Lip-Sync Video", visible=False)
            ...
            def generate_audio(ref_audio_input, gen_text_input):
                ...
                return generated_audio, spectrogram_path, True, False

            def enable_lip_sync(checked):
                return gr.update(visible=checked), gr.update(visible=checked)

            def perform_lip_sync(video_input, audio_output):
                output_video = lip_sync(video_input.name, audio_output[1])
                return output_video
            ...
            generate_btn.click(
                ...
                outputs=[audio_output, spectrogram_output, lip_sync_option, video_input]
            )
