...
def interface():
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            lip_sync_button = gr.Button("Generate Lip Sync", visible=False)
        ...
        def show_video_input(lip_sync):
            return gr.update(visible=lip_sync), gr.update(visible=lip_sync), gr.update(visible=lip_sync)

        # Define the function for voice cloning and lip sync
        def generate_and_sync(ref_audio, ref_text, gen_text, remove_silence, lip_sync_option, video_file):
            cloned_audio_path = generate_cloned_voice(ref_audio, ref_text, gen_text, remove_silence)
            lip_synced_video_path = None
            if lip_sync_option and video_file:
                lip_synced_video_path = lip_sync(video_file, cloned_audio_path)
            return cloned_audio_path, lip_synced_video_path if lip_sync_option else gr.update(visible=False)

        # Show video input, button, and output based on lip sync checkbox
        lip_sync_option.change(show_video_input, inputs=lip_sync_option, outputs=[video_input, lip_sync_button, lip_sync_output])

        # Trigger cloning and optional lip sync with video
        clone_button.click(
            fn=generate_and_sync,
            inputs=[ref_audio_input, ref_text_input, gen_text_input, remove_silence, lip_sync_option, video_input],
            outputs=[cloned_audio_output, lip_sync_output]
        )
